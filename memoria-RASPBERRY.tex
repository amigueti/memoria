...
\newpage
\chapter*{Summary}
Since their appearance on scene, electronics such as \texttt{Arduino} or \texttt{Raspberry Pi} have completely refefined the attitude of average users to embedded systems. Due to its low price, simplicity and multiple possibilities, this electronics have become a very popular piece among the online community of programmers.
\newline
\newline
...
\newline
\newline
This project attempts to enter these three worlds. We have designed a system that allows the acquisition of data through sensors connected to a  \texttt{Raspberry Pi}. It also provides the system with tools to manipulate other electrical circuits such as relays, motors, etc. All this is presented to the user as an intuitive web application easy to use.
...
\begin{enumerate}
	...
	En este primer capítulo hemos explicado el problema, la mativación que nos mueve a abordarlo y los objetivos marcados para su desarrollo.
	
	\item \textit{Estado del Arte}\newline
	A continuación describimos la actualidad de la tecnología usada en el desarrollo de la aplicación y esta misma memoria. Se hace especial hincapié en la descripción de la \texttt{Raspberry Pi} pués es el eje central de todo el proyecto.
	
	\item \textit{Implementación y Desarrollo del Proyecto}\newline
	En este capítulo se describe la aplicación diseñada. Se mencionan los módulos que la componen, su estructura y su funcionamiento. Al final del capítulo se describen un par de aplicaciones desarrollas con las herramientas implementadas.
	
	\item \textit{Conclusiones}\newline
	Al final de esta memoria se encuentran las conclusiones extraidas tanto del proyecto como de la \texttt{Raspberry Pi}. También se hace un repaso de los conocimientos necesarios así como los aprendidos.
\end{enumerate}
...
\subsection{Raspberry Pi GPIO}\label{gpio}
...
\subsection{\LaTeX}
\LaTeX \space es un sistema de composición de textos de alta calidad. Es un sistema muy diferente a la gran mayoría de editores de texto convencionales del tipo `What You See Is What You Get' o `lo que ves es lo que obtienes'. Se compone de comandos \TeX \space con los que el autor puede elegir el tipo de documento a realizar y no preocuparse tanto por el estilo del mismo. Con ello la publicación de textos de alta calidad se vuelve más sencilla.
...
\subsection{LED y Sensor de Temperatura}\label{subsec:termometro}
...
\subsection{Riego}\label{subsec:riego}
...
\begin{enumerate}
  \item \textit{Electrónica y Sistemas Digitales.}\newline
  ...
  \item \textit{Servicios y Aplicaciones en Redes de Ordenadores.}\newline
  En esta asignatura se enseña lo básico sobre los servicios y aplicaciones comunes en las redes de ordenadores, y en particular en internet \cite{saro}. En ella aprendí a manejar el framework Django que ha permitido realizar en este proyecto la parte del servidor. Así mismo también se enseñan a realizar los \texttt{templates} que componen la vista de la aplicación web y a dotarlos de estilo con la programación \texttt{CSS}.
  
  \item \textit{Comunicaciones Sectoriales.}\newline
  Esta asignatura se imparte en el últiumo semestre de la carrera. Su finalidad no es enseñar al alumno más conocimientos sino darle una imagen práctica de los conocimientos ya adquiridos. Es decir, aquí se presentan diversas tecnologías que se componen de lo aprendido en el grado hasta el momento. Esta asignatura motivó en gran parte este proyecto. 
\end{enumerate}
...
\begin{enumerate}
  \item \textit{Protocolos de Comunicación.}\newline
  Durante el diseño de los circuitos fue necesarios entender como funcionaban los diversos protocolos de comunicación mencionados en la sección \ref{gpio}.
  
  \item \textit{Desarrollo de un Proyecto.}\newline
  Es importante mencionar que también se ha adquirido experiencia en el desarrollo de un proyecto que auna tantas piezas. Desde su diseño hasta su final implementación se ha tenido que lidiar con multitud de herramientas de distintas índoles. Por lo que fue necesario una planificacíon previa de los pasos a seguir para completar el proyecto.
  
  \item \textit{Manejo de \LaTeX.}\newline
...
Una vez finalizado el proyecto es posible plantearse una serie de mejoras o extras que podrían dotar a la \texttt{Raspberry Pi} de mayores funciones.
\newline
\newline
Quizás los componentes que mayor juego pueden darle a este proyecto serían los módulos de comunicación. Dotando al sistema de dichos módulos podríamos tener un sistema que no solo se pueda aplicar en un area pequeña, como eran el termómetro \ref{subsec:termometro} o el riego \ref{subsec:riego}, sino en un area de mayor extensión. Así mismo sería interesante añadir la capacidad de mostrar información en displays.
\newline
\newline
En cuanto a mejoras del software cabe mencionar la posiblidad de añadir más herramientas que permitan al usuario realizar distintas acciones con los circuitos y así mismo añadan más funciones a los sistemas implementados. También podría mejorarse la interfaz.
...
\bibitem{saro} Dr. Jesús M. González Barahona, Dr. Gregorio Robles Martínez \textit{Servicios y Aplicaciones en Redes de Ordenadores Grado en Sistemas de Telecomunicación Programa del curso}
